    ...
    try {
        ...
        const filteredStyles = currentStyles.filter(style => {
            if (typeof style === 'string') {
                ...
                return !lowerStyle.includes('markdown-navigator') &&
                    ...
                    !lowerStyle.includes('cfml-highlight.js') &&
        ...
        try {
            ...
            if (workspaceFolders && workspaceFolders.length > 0) {
                ...
                stylesDir = path.join(context.extensionPath, 'styles-temp');
            }
            ...
            if (!customCssPath) {
                ...
                if (currentTheme === 'default') {
                    // Use default.css for the default theme
                    themeCssPath = path.join(context.extensionPath, 'styles', 'default.css');
                    destThemePath = path.join(stylesDir, 'default.css');
                } else {
                    // Try the specified theme
                    themeCssPath = path.join(context.extensionPath, 'styles', `${currentTheme}.css`);
                    destThemePath = path.join(stylesDir, `${currentTheme}.css`);
                    
                    // If the theme file doesn't exist, fall back to default.css
                    if (!fs.existsSync(themeCssPath)) {
                        console.warn(`[Markdown Navigator] Theme CSS file not found: ${themeCssPath}, falling back to default.css`);
                        themeCssPath = path.join(context.extensionPath, 'styles', 'default.css');
                        destThemePath = path.join(stylesDir, 'default.css');
                    }
                }
                ...
                if (fs.existsSync(themeCssPath)) {
                    ...
                    if (workspaceFolders && workspaceFolders.length > 0) {
                        ...
                        styles.push(destThemePath);
                        console.log(`[Markdown Navigator] Added theme CSS with absolute path: ${destThemePath}`);
                    }
                } else {
                    console.error(`[Markdown Navigator] Default CSS file not found: ${themeCssPath}`);
            ...
            if (enableCfmlHighlighting) {
                const cfmlCssPath = path.join(context.extensionPath, 'styles', 'cfml-syntax.css');
                const destCfmlPath = path.join(stylesDir, 'cfml-syntax.css');
                
                if (fs.existsSync(cfmlCssPath)) {
                    fs.copyFileSync(cfmlCssPath, destCfmlPath);
                    
                    // Add relative path to styles
                    if (workspaceFolders && workspaceFolders.length > 0) {
                        const relativePath = path.relative(workspaceFolders[0].uri.fsPath, destCfmlPath);
                        const dotRelativePath = `./${relativePath.replace(/\\/g, '/')}`;
                        styles.push(dotRelativePath);
                        console.log(`[Markdown Navigator] Added CFML CSS with relative path: ${dotRelativePath}`);
                    } else {
                        styles.push(destCfmlPath);
                        console.log(`[Markdown Navigator] Added CFML CSS with absolute path: ${destCfmlPath}`);
                    }
                }
            }
            
            // Handle custom CSS if specified
            if (customCssPath && customCssPath.length > 0) {
                ...
                if (fs.existsSync(resolvedPath)) {
                    const destCustomPath = path.join(stylesDir, path.basename(customCssPath));
                    fs.copyFileSync(resolvedPath, destCustomPath);
                    
                    // Add relative path to styles
                    if (workspaceFolders && workspaceFolders.length > 0) {
                        const relativePath = path.relative(workspaceFolders[0].uri.fsPath, destCustomPath);
                        const dotRelativePath = `./${relativePath.replace(/\\/g, '/')}`;
                        styles.push(dotRelativePath);
                        console.log(`[Markdown Navigator] Added custom CSS with relative path: ${dotRelativePath}`);
                    } else {
                        styles.push(destCustomPath);
                        console.log(`[Markdown Navigator] Added custom CSS with absolute path: ${destCustomPath}`);
                    }
                } else {
        ...
        vscode.window.showInformationMessage(`Applied ${currentTheme} theme with ${styles.length} style sheets`, 'View Details').then(selection => {
            if (selection === 'View Details') {
                // Create a human-readable list of styles
                const styleDetails = styles.map(s => `- ${s}`).join('\n');
                const message = `Applied theme: ${currentTheme}\n\nStyle sheets:\n${styleDetails}`;
                vscode.window.showInformationMessage(message);
            }
        });
